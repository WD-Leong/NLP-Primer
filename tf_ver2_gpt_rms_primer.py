...
class RMSNormalization(tf.keras.layers.Layer):
    ...
    def call(self, x):
        x_irms = tf.math.rsqrt(tf.reduce_mean(
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, v, k, q, mask=None):
        ...
        q_pad = tf.expand_dims(tf.concat(
            [x_zero_pad, self.wq(q)], axis=1), axis=1)
        k_pad = tf.expand_dims(tf.concat(
            [x_zero_pad, self.wk(k)], axis=1), axis=1)
        v_pad = tf.expand_dims(tf.concat(
            [x_zero_pad, self.wv(v)], axis=1), axis=1)
        
        q_input = self.split_heads(
            tf.squeeze(self.depth_cnn_q(q_pad), axis=1))
        k_input = self.split_heads(
            tf.squeeze(self.depth_cnn_k(k_pad), axis=1))
        v_input = self.split_heads(
            tf.squeeze(self.depth_cnn_v(v_pad), axis=1))
        
        attn_tuple = scaled_dot_product_attention(
            q_input, k_input, v_input, mask=mask, neg_infty=-1.0e9)
...
class DecoderLayer(tf.keras.layers.Layer):
    def __init__(
        ...
        self.lnorm_1 = RMSNormalization(eps=1.0e-6)
        self.lnorm_2 = RMSNormalization(eps=1.0e-6)
