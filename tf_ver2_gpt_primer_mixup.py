...
class Decoder(tf.keras.layers.Layer):
    ...
    def mixup_output(self, x1, x2, alpha, training=True):
        seq_length = tf.shape(x1)[1]
        input_mask = tf.linalg.band_part(
            tf.ones([seq_length, seq_length]), -1, 0)
        input_mask = 1.0 - input_mask
        
        x1_tok_embed = self.dec_embed(x1)
        x2_tok_embed = self.dec_embed(x2)
...
class PrimerDecoder(tf.keras.Model):
    ...
    def mixup_output(
        ...
        dec_outputs = self.gpt_model.mixup_output(
            x1, x2, alpha, training=training)
        dec_logits  = self.p_decoder(dec_outputs)
        return dec_logits
