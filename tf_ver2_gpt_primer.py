...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, v, k, q, mask=None):
        ...
        attn_tuple = scaled_dot_product_attention(
            q, k, v, mask=mask, neg_infty=-1.0e9)
...
class DecoderLayer(tf.keras.layers.Layer):
    def __init__(
        ...
        self.depth_cnn = DepthwiseConv2D(
    ...
    def call(
        ...
        attn_depth_input = tf.expand_dims(tf.concat(
            [x_zero_pad, attn_self_output], axis=1), axis=1)
        attn_depth_output = self.depth_cnn(attn_depth_input)
        attn_depth_output = tf.squeeze(attn_depth_output, axis=1)
        
        # Feed Forward Layer. #
        ffwd_self_output = self.lnorm_2(
            self.ffwd_self(attn_depth_output))
